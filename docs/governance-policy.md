...
**Correction -- the matching `file_read` exception was claimed but never implemented:** commit `723d026`'s message said it added a "matching file_read exception for .memory/project/", but its diff to `coursetools_server.py` only changed `file_write` (plus the allow-list path default). `file_read` still blocked all of `.memory/` except `.memory/knowledge/`, so the role could write `.memory/project/` files but not read them. The first real run after `723d026` hit this directly: asked to check `MEMORY_INDEX.md` for stale or placeholder entries, `decision-auditor` was refused on `file_read` of `MEMORY_INDEX.md` and on `codebase_search` under `.memory/project`, and -- correctly, per its own rule never to write a file it hasn't read -- made no changes (transcript: `.eval-artifacts/runs/decision-auditor-first-attempt.json`). The fix adds the exception to `file_read` using the same `in_project and role == "decision-auditor"` pattern as `file_write`: `.memory/project/` is readable for `role="decision-auditor"` only; `.memory/storage/`, `.memory/reference/`, and `.memory/project/` for every other role stay blocked. `.memory/knowledge/` read access is unchanged (still directly readable, as before). `codebase_search` still skips all of `.memory/` for every role -- not needed for this role's job, so deliberately not widened.

## Deterministic conversion history

- `orchestrator_test`: converted from Orchestrator narrative interpretation to deterministic code, see ADR-001 (docs/adr/ADR-001-orchestrator-test-deterministic-conversion.md). No MCP allow-list grant is removed by this conversion -- this step never called an MCP tool; it ran a shell command and reasoned about the output directly, which is exactly the part now replaced by scripts/parse_test_result_deterministic.py.
